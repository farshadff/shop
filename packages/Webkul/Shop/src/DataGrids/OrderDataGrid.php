        ...
        $this->addColumn([
            ...
            'wrapper' => function ($value) {
                if ($value->status == 'processing')
                    return '<span class="badge badge-md badge-success">در حال انجام</span>';
                else if ($value->status == 'completed')
                    return '<span class="badge badge-md badge-success">تکمیل شده</span>';
                else if ($value->status == "canceled")
                    return '<span class="badge badge-md badge-danger">کنسل شده</span>';
                else if ($value->status == "closed")
                    return '<span class="badge badge-md badge-info">بسته شده</span>';
                else if ($value->status == "pending")
                    return '<span class="badge badge-md badge-warning">در حال انتظار</span>';
                else if ($value->status == "pending_payment")
                    return '<span class="badge badge-md badge-warning">در انتظار پرداخت</span>';
                else if ($value->status == "fraud")
                    return '<span class="badge badge-md badge-danger">جعلی</span>';
            },
            'filterable' => true
        ]);
    }
